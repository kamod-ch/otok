...
export const action = defineAction({
  ...
  handler: async ({ input, hono, db }) => {
    ...
    const database = db as import("kysely").Kysely<DevjobsDatabase>;
...
export default function LoginPage({ data, actionData }: OtokPageProps<any>) {
  ...
  return (
    <AppShell title="Devjobs Reference" i18n={data.i18n}>
      <section class="mx-auto grid max-w-md gap-6 px-4 py-16 sm:px-6">
        <div class="space-y-2">
        ...
        <form method="post" class="grid gap-4 rounded-xl border border-border bg-card p-6 shadow-sm">
          ...
          <FormField
            ...
            required
          />
          <FormActions submitLabel={data.copy.submit} cancelHref="/jobs" />
        </form>
      </section>
    </AppShell>
  );
