...
export const loader = defineLoader<{ contact: import("../../../../db/types.js").Contact }, ContactsDatabase>(
  async ({ db, params }) => {
...
export const action = defineAction({
  ...
  handler: async ({ input, db, params }) => {
    ...
    const { id } = await parseParams(params, contactIdSchema);
    await db.updateTable("contacts").set({ name: input.name, email: input.email }).where("id", "=", id).execute();
...
export default function EditContact({
  data,
  actionData,
}: OtokPageProps<{ contact: { id: number; name: string; email: string } }>) {
  ...
  return (
    <section>
      ...
      <form method="post" action={`/contacts/${data.contact.id}/delete`} style="margin-top:2rem;">
        <button
          type="submit"
          style="padding:0.5rem 1rem;background:#dc2626;color:white;border:none;border-radius:0.375rem;"
        >
          Delete contact
        </button>
      </form>
