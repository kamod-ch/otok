...
export function createOtokSupabaseBrowserClient<Database>(config: SupabaseConfig): SupabaseClient<Database> {
  const validated = validateSupabaseConfig(config);

  if (typeof window === "undefined") {
    throw new Error("otok-supabase: createOtokSupabaseBrowserClient() must only be called in the browser.");
  }
